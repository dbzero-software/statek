...
class TestExecAllSteps:
    ...
    @pytest.mark.asyncio
    async def test_exit_in_cli_step(self, job_factory):
        """exit() in a CLI step signals completion."""
        job = self.create_job(job_factory)
        cs = CallSpec(id="1", func_name="python_cli", kwargs={"code": 'exit("done")'})
        block = CodeBlock(code=None, tool_calls=[cs])
        result = await exec_all_steps(block, job, lambda idx, s: None)
        assert result is True
    ...
    async def test_continuation_with_tuple_instr_num(self, job_factory):
        """A tuple instr_num skips regular code and resumes at the right instruction."""
        job = self.create_job(job_factory)
        cs = CallSpec(id="a", func_name="python_cli", kwargs={"code": 'b = 2\nb = 3'})
        block = CodeBlock(code='x = 99', tool_calls=[cs])
        # Continue from cli step 0, instruction 1 (skip b=2, run b=3)
        await exec_all_steps(block, job, lambda idx, s: None, instr_num=(0, 1))
    ...
    @pytest.mark.asyncio
    async def test_future_error_in_cli_step(self, job_factory):
        """FutureError in a CLI step carries a (cli_step_index, instr_num) tuple."""
        job = self.create_job(job_factory)
        job.py_env.local_state = {'get_value_not_ready': get_value_not_ready}
        cs = CallSpec(id="a", func_name="python_cli",
                      kwargs={"code": "result = get_value_not_ready()\nprint(result)"})
        block = CodeBlock(code=None, tool_calls=[cs])
    ...
    async def test_continuation_resumes_blocked_cli_step(self, job_factory):
        """Tuple instr_num resumes at the correct instruction within a CLI step."""
        job = self.create_job(job_factory)
        ...
        # First run — blocks at cli step 0, instruction 1
        with pytest.raises(FutureError) as exc_info:
            await exec_all_steps(block, job, lambda idx, s: None)
        assert exc_info.value.instr_num == (0, 1)
...
class TestSingleCliCallLimit:
    """exec_all_steps must raise ValueError when more than 1 python_cli call is present."""

    def create_job(self, job_factory, job_params=None):
        return job_factory(job_params=job_params or DEFAULT_JOB_PARAMS)

    @pytest.mark.asyncio
    async def test_raises_on_multiple_cli_calls(self, job_factory):
        """Two python_cli calls in a single code block must raise ValueError."""
        job = self.create_job(job_factory)
        cs0 = CallSpec(id="a", func_name="python_cli", kwargs={"code": 'x = 1'})
        cs1 = CallSpec(id="b", func_name="python_cli", kwargs={"code": 'y = 2'})
    ...
    @pytest.mark.asyncio
    async def test_raises_on_three_cli_calls(self, job_factory):
        """Three python_cli calls also trigger the ValueError."""
        job = self.create_job(job_factory)
        cs0 = CallSpec(id="a", func_name="python_cli", kwargs={"code": 'x = 1'})
        cs1 = CallSpec(id="b", func_name="python_cli", kwargs={"code": 'y = 2'})
        cs2 = CallSpec(id="c", func_name="python_cli", kwargs={"code": 'z = 3'})
        block = CodeBlock(code=None, tool_calls=[cs0, cs1, cs2])

        with pytest.raises(ValueError, match="python_cli"):
            await exec_all_steps(block, job, lambda idx, s: None)

    @pytest.mark.asyncio
    async def test_single_cli_call_allowed(self, job_factory):
        """A single python_cli call executes normally without error."""
        job = self.create_job(job_factory)
        cs = CallSpec(id="a", func_name="python_cli", kwargs={"code": 'x = 42'})
        block = CodeBlock(code=None, tool_calls=[cs])
    ...
    @pytest.mark.asyncio
    async def test_no_code_executed_on_violation(self, job_factory):
        """Neither CLI call's code runs when the limit is violated."""
        job = self.create_job(job_factory)
        cs0 = CallSpec(id="a", func_name="python_cli", kwargs={"code": 'x = 1'})
        cs1 = CallSpec(id="b", func_name="python_cli", kwargs={"code": 'y = 2'})
        block = CodeBlock(code=None, tool_calls=[cs0, cs1])
        ...
        assert job.py_env.local_state.get('x') is None
        assert job.py_env.local_state.get('y') is None
